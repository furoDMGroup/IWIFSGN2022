...
ozone = pd.read_csv(pth.concatenate_path_os_independent('eighthr.data'), header=None)
ozone = ozone.where(ozone != '?', None)
ozone = ozone.dropna()

X = ozone.iloc[:, 1:-1].to_numpy()
y = ozone.iloc[:, -1].to_numpy()
    ...
    for k in ks:
        for agg in aggs:
            ...
            f_result = cross_validate_missing_in_test(binaryF, X, y,
                                                      scoring='roc_auc', return_estimator=True, cv=skf, inserter=inserter, dataset='ozone')
...
concatenated.to_excel('concatenated_ozone.xlsx')