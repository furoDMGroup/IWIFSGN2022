...
german = pd.read_csv(pth.concatenate_path_os_independent('german.data-numeric.csv'), header=None, sep=' ')

X = german.iloc[:, :-1].to_numpy()
y = german.iloc[:, -1].to_numpy()
    ...
    for k in ks:
        for agg in aggs:
            ...
            f_result = cross_validate_missing_in_test(binaryF, X, y,
                        scoring='roc_auc', return_estimator=True, cv=skf, inserter=inserter, dataset='german')
...
concatenated.to_excel('concatenated_german.xlsx')
