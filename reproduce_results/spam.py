...
spam = pd.read_csv(pth.concatenate_path_os_independent('spambase.data'), header=None)

X = spam.iloc[:, :-1].to_numpy()
y = spam.iloc[:, -1].to_numpy()
    ...
    for k in ks:
        for agg in aggs:
            ...
            f_result = cross_validate_missing_in_test(binaryF, X, y,
                                                      scoring='roc_auc', return_estimator=True, cv=skf, inserter=inserter, dataset='spam')
...
concatenated.to_excel('concatenated_spam.xlsx')
